...
def generate(input_dir, output_dir, biotea_command):
    if biotea_command == 'metadata':
        biotea_command = ['java', '-Xmx2048m', '-jar', 'biotea_rdfization.jar', '-in', input_dir + 'in_process', '-out', output_dir]
    else:
        biotea_command = ['java', '-Xmx2048m', '-jar', 'biotea_annotation.jar', '-in', input_dir + 'in_process', '-out', output_dir,
